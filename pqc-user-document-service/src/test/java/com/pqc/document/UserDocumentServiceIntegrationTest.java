...
class UserDocumentServiceIntegrationTest {
    ...
    void testAdminHasQuantumResistantKeys() throws Exception {
        ...
        System.out.println("✅ Admin has quantum-resistant algorithm preferences:");
        System.out.println("   Signature: ML_DSA (CRYSTALS-Dilithium)");
        System.out.println("   Encryption: ML_KEM (CRYSTALS-Kyber)");
    }
    ...
    void testDocumentSigningWithMLDSA() throws Exception {
        MvcResult result = mockMvc.perform(post("/api/documents/" + documentId + "/sign")
                .header("Authorization", "Bearer " + citizenToken))
                .andExpect(status().isOk())
        ...
        System.out.println("   Signature Size: ~2,420 bytes");
    }
    ...
    void testDocumentVerification() throws Exception {
        MvcResult result = mockMvc.perform(post("/api/documents/" + documentId + "/verify")
                .header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isOk())
        ...
        System.out.println("✅ Document signature verified:");
        System.out.println("   Valid: YES");
        System.out.println("   Quantum Threat Level: NONE (Protected)");
    }
    ...
    void testDocumentWithRSASignature() throws Exception {
        ...
        // Sign with RSA
        MvcResult signResult = mockMvc.perform(post("/api/documents/" + rsaDocId + "/sign")
                .header("Authorization", "Bearer " + citizenToken))
                .andExpect(status().isOk())
        ...
        System.out.println("⚠️ Document signed with RSA-2048 (VULNERABLE):");
        System.out.println("   Algorithm: RSA-2048");
        System.out.println("   Quantum Resistant: NO");
        System.out.println("   Threat: Can be broken by Shor's algorithm in ~8 hours on quantum computer");
